...
function initializeDashboard(user) {
    ...
    if(document.getElementById('profileBatch')) document.getElementById('profileBatch').textContent = (user.batch === 'PRIVATE_CANDIDATE') ? 'Private Candidate' : user.batch;
    ...
    const myBatchExams = allExams.filter(exam => 
        !exam.isPrivate && 
        (!exam.assignedBatches || exam.assignedBatches.includes('ALL') || exam.assignedBatches.includes(user.batch))
    );

    const takenExamIds = new Set(myResults.map(r => r.examId));
    const pendingExams = myBatchExams.filter(exam => !takenExamIds.has(exam.id));

    updateStats(myResults, pendingExams);
    renderAvailableExams(pendingExams);
    renderRecentActivity(myResults, allExams);
    drawPerformanceChart(myResults);
}

function updateStats(myResults, pendingExams) {
    if(document.getElementById('examsTakenCount')) document.getElementById('examsTakenCount').textContent = myResults.length;
    if(document.getElementById('pendingExamsCount')) document.getElementById('pendingExamsCount').textContent = pendingExams.length;

    if (myResults.length > 0) {
        const totalPercentage = myResults.reduce((sum, r) => sum + ((r.score / r.totalMarks) * 100), 0);
        const average = Math.round(totalPercentage / myResults.length);
        if(document.getElementById('averageScore')) document.getElementById('averageScore').textContent = `${average}%`;
    } else {
        if(document.getElementById('averageScore')) document.getElementById('averageScore').textContent = "-";
    }
}

function renderAvailableExams(pendingExams) {
    const listContainer = document.getElementById('availableExamsList');
    if (!listContainer) return;
    listContainer.innerHTML = '';

    const now = new Date();
    const openExams = pendingExams.filter(exam => {
    ...
    if (openExams.length === 0) {
        const upcoming = pendingExams.some(e => e.startDate && new Date(e.startDate) > now);
        listContainer.innerHTML = upcoming 
            ? '<div class="empty-state">⏳ No exams open right now. Check back later!</div>'
            : '<div class="empty-state">🎉 You are all caught up!</div>';
        return;
    }

    openExams.slice(0, 3).forEach(exam => {
        listContainer.innerHTML += `
            <div class="exam-card">
                <div class="exam-info">
                    <h4>${exam.title}</h4>
                    <p><span class="status-pending-tag">${exam.subject}</span> • ${exam.duration} Mins</p>
                </div>
                <a href="exams.html?start=${exam.id}" class="btn btn-primary btn-small">Start</a>
            </div>`;
    });
}
...
function renderRecentActivity(myResults, allExams) {
    ...
    if (myResults.length === 0) {
        listContainer.innerHTML = '<div class="empty-state">No exams taken yet.</div>';
        return;
    }
    ...
    recent.forEach(r => {
        ...
        listContainer.innerHTML += `
            <div class="exam-card">
                <div class="exam-info"><h4>${exam.title}</h4><p>${new Date(r.dateTaken).toLocaleDateString()}</p></div>
...
function drawPerformanceChart(myResults) {
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            scales: { y: { beginAtZero: true, max: 100, ticks: { display: false } }, x: { ticks: { display: false } } },
...
function joinPrivateExam() {
    ...
    if (exam) {
        ...
        if (alreadyTaken) {
            alert("You have already taken this private exam.");
        } else {
            if(confirm(`Found Exam: "${exam.title}"\nDo you want to start it now?`)) {
                window.location.href = `exams.html?start=${exam.id}`;
            }
        }
    } else {
...
function loadStudentResults() {
    ...
    if (myResults.length === 0) {
        container.innerHTML = '<div class="empty-state">You haven\'t taken any exams yet.</div>';
        return;
    }
    ...
    myResults.forEach(result => {
        ...
        if (result.status === 'flagged') {
            statusBadge = `<span style="background:#fee2e2; color:#991b1b; padding:4px 12px; border-radius:20px; font-weight:700;">⚠️ FLAGGED (Violation)</span>`;
            cardStyle = 'border-left: 4px solid #ef4444;';
        } else if (result.needsGrading) {
            statusBadge = `<span style="background:#fef3c7; color:#d97706; padding:4px 12px; border-radius:20px; font-weight:600;">⏳ Pending Grading (${result.score}/${result.totalMarks})</span>`;
        }

        const percentage = (!result.needsGrading && result.status !== 'flagged') ? Math.round((result.score / result.totalMarks) * 100) : 0;

        container.innerHTML += `
            <div class="exam-card" style="${cardStyle} display:block;">
                <div style="display:flex; justify-content:space-between; align-items:start; margin-bottom:1rem;">
                    <div>
                        <h3 style="margin-bottom:0.5rem;">${exam.title}</h3>
                        <p style="color:#64748b;">${exam.subject} • Taken on ${new Date(result.dateTaken).toLocaleDateString()}</p>
                    </div>
                    <div style="text-align:right;">${statusBadge}</div>
                </div>
                ${(!result.needsGrading && result.status !== 'flagged') ? `
                <div>
                    <div style="display:flex; justify-content:space-between; font-size:0.9rem; margin-bottom:0.25rem; color:#64748b;"><span>Performance</span><span>${percentage}%</span></div>
                    <div class="progress-bar"><div class="progress-fill" style="width: ${percentage}%"></div></div>
                </div>` : ''}
                <div style="margin-top: 1rem; text-align: right;">
                     <button onclick="viewResultDetails('${result.resultId}')" class="btn btn-secondary btn-small" ${result.status === 'flagged' ? 'disabled' : ''}>
                        View Details
                     </button>
                </div>
            </div>`;
    ...
    exam.questions.forEach((q, idx) => {
        ...
        if (q.type === 'mcq') {
            ...
            statusBadge = isCorrect 
                ? `<span class="result-badge badge-correct">Correct (+${q.marks})</span>`
                : `<span class="result-badge badge-incorrect">Incorrect (0/${q.marks})</span>`;
            answerDisplay = (userAnswer !== undefined && q.options[userAnswer]) ? q.options[userAnswer] : 'Skipped';
        } else {
            statusBadge = result.needsGrading 
                 ? `<span class="result-badge badge-pending">Pending Grading</span>`
                 : `<span class="result-badge" style="background:#e2e8f0">Manually Graded</span>`;
        }

        container.innerHTML += `
            <div class="result-q-card">
                <p><strong>Q${idx+1}:</strong> ${q.text}</p>
                <p style="margin-top:0.5rem; color:#64748b; font-size:0.9rem;">Your Answer:</p>
                <div style="background:white; padding:0.8rem; border-radius:8px; border:1px solid #cbd5e1; margin-top:0.25rem;">
                    ${answerDisplay}
                </div>
